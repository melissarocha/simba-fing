...
if ( !empty($_POST) ) {
    ...
    $matricula_compara = $data['matricula'];
    Database::disconnect();
    ...
    $valid = true;
    if ( $count > 0  && ($matricula_compara != $matricula) ) { // En caso de encontrar registro duplicados...
    ...
    if ( $valid ) {
        $pdo = Database::connect();
        $pdo -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "UPDATE alumnos SET matricula = ?, nombre_alumno = ?, apellido_paterno = ?, apellido_materno = ?, correo_electronico = ?, id_beca = ?, id_carrera = ?, id_estado = ? WHERE id_alumno = ?;";
        $q = $pdo -> prepare($sql);
        $q -> execute(array($matricula, $nombre_alumno, $apellido_paterno, $apellido_materno, $correo_electronico, $id_beca, $id_carrera, $id_estado, $id_alumno));
        Database::disconnect();
        header("Location: alumnos.php");
    }
} else {
...
<div class="container">
    <div class="span10 offset1">
        ...
        <form class="form-horizontal" action="edit.php?id_alumno=<?php echo $id_alumno?>" method="post">
